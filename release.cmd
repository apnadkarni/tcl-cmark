...
@if NOT "x%package%" == "x" goto gettclname
@echo Could not get package name!
@goto abort

:gettclname
@for /F "usebackq tokens=2 delims=="  %%i in (`findstr "PACKAGE_TCLNAME=" %configure%`) do @set tclname=%%i
...
echo lappend auto_path %outdiru%; exit [catch {puts [package require %tclname%]}] | %dir90x64%\bin\tclsh90.exe
@if ERRORLEVEL 1 goto abort

echo lappend auto_path %outdiru%; exit [catch {puts [package require %tclname%]}] | %dir90x86%\bin\tclsh90.exe
@if ERRORLEVEL 1 goto abort

echo lappend auto_path %outdiru%; exit [catch {puts [package require %tclname%]}] | %dir86x64%\bin\tclsh86t.exe
@if ERRORLEVEL 1 goto abort

echo lappend auto_path %outdiru%; exit [catch {puts [package require %tclname%]}] | %dir86x86%\bin\tclsh86t.exe
@if ERRORLEVEL 1 goto abort
